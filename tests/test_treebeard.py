...
@pytest.mark.django_db
class TestMP_TreeFindProblemsForTree(TestTreeBase):
    def test_find_problems_for_tree_not_root(self, mpalphabet_model):
        mpalphabet_model.alphabet = "01234"
        mpalphabet_model(path="01", depth=1, numchild=1, numval=0).save()
        mpalphabet_model(path="0101", depth=2, numchild=0, numval=0).save()
        node = mpalphabet_model.objects.get(path="0101")
        with pytest.raises(ValueError):
            mpalphabet_model.find_problems_for_tree(node)

    def test_find_problems_for_tree_clean(self, mpalphabet_model):
        mpalphabet_model.alphabet = "01234"
        mpalphabet_model(path="01", depth=1, numchild=1, numval=0).save()
        mpalphabet_model(path="0101", depth=2, numchild=0, numval=0).save()
        root = mpalphabet_model.objects.get(path="01")
        result = mpalphabet_model.find_problems_for_tree(root)
        assert all(not group for group in result)

    def test_find_problems_for_tree_scoped_to_single_tree(self, mpalphabet_model):
        ...
        result1 = mpalphabet_model.find_problems_for_tree(root1)
        assert all(not group for group in result1)

        # Tree 2 should have problems
        result2 = mpalphabet_model.find_problems_for_tree(root2)
    ...
    def test_find_problems_for_tree_detects_all_problem_types(self, mpalphabet_model):
        ...
        evil_chars, bad_steplen, orphans, wrong_depth, wrong_numchild = mpalphabet_model.find_problems_for_tree(root)
        # "03" reports numchild=2 but only has 1 direct child -> wrong_numchild
        assert "03" in got(wrong_numchild)
        # "030102" is an orphan (parent "0301" exists but "030102"'s depth=3
        # doesn't match its path len 6 / steplen 2 = 3, actually matches,
        # but "030102" reports numchild=10 which is wrong)
